...
impl ResidentAddressProjection {
    ...
    /// so a contiguous DocId range spans a contiguous address interval. A
    /// cross-column scan that seeks by address relies on both halves; see
    /// [`AddressKeyedDocuments::addresses_strictly_ascending`].
    fn addresses_strictly_ascending(&self) -> bool {
        // A remapper is attached whenever the dataset carries a fragment reuse
        // index, even when it retains every document, so liveness has to be
        // decided on cardinality rather than on the presence of `live_docs`.
        self.projection
            .live_docs
            .as_ref()
            .is_none_or(|live| live.len() == self.len() as u64)
            && (1..self.len())
                .all(|index| self.stored_address(index - 1) < self.stored_address(index))
    }
...
pub(super) struct PartitionDocuments {
    ...
    /// Answer to [`AddressKeyedDocuments::addresses_strictly_ascending`], a
    /// property of the immutable address column, so it is computed once.
    strictly_ascending_addresses: OnceCell<bool>,
}
...
impl PartitionDocuments {
    pub(crate) fn try_new(
        ...
        Ok(Self {
            ...
            strictly_ascending_addresses: OnceCell::new(),
        })
    }
    ...
    pub(crate) fn prewarm_status(&self) -> FtsPrewarmDocumentStatus {
        FtsPrewarmDocumentStatus {
            ...
            ascending_addresses_ready: self.strictly_ascending_addresses.initialized(),
    ...
    /// Memoized answer to [`AddressKeyedDocuments::addresses_strictly_ascending`].
    /// The scan is O(num_docs), so it belongs to prewarmed partition state.
    async fn strictly_ascending_addresses(
        &self,
        projection: &ResidentAddressProjection,
    ) -> Result<bool> {
        self.strictly_ascending_addresses
            .get_or_try_init(|| {
                let projection = projection.clone();
                async move {
                    spawn_cpu(move || Result::Ok(projection.addresses_strictly_ascending())).await
                }
            })
            .await
            .copied()
...
mod tests {
    ...
    fn ascending_address_gate_requires_live_slots_and_strict_order(
        ...
        assert_eq!(projection.addresses_strictly_ascending(), expected);
    }
    ...
    async fn prewarm_loads_document_columns_and_address_lookup_once() {
        ...
        assert_eq!(documents.strictly_ascending_addresses.get(), Some(&true));
    }
    ...
    async fn prewarm_answers_the_ascending_gate_for_a_remapped_partition(
        ...
        assert!(documents.query_ready());
        assert_eq!(
            documents.strictly_ascending_addresses.get(),
