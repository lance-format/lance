...
impl StablePartitionMapping {
    ...
    /// Load every row-map chunk into the index cache concurrently. Opens the
    /// row-map file on first use. A no-op when no block cache is attached.
    pub async fn prewarm(&self) -> Result<()> {
        if self.block_cache.is_none() {
            return Ok(());
        }
        let reader = self.open_reader().await?;
        reader.prewarm().await
    }
