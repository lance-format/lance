...
/// Concurrent chunk loads issued by [`RowMapReader::prewarm`]. Bounds in-flight
/// ~4 MiB reads so a whole-file prewarm does not swamp the store.
const ROW_MAP_PREWARM_CONCURRENCY: usize = 16;
...
impl RowMapReader {
    ...
    /// The chunk index one past the last chunk (number of chunks).
    fn num_chunks(&self) -> usize {
        self.counts
            .num_blocks()
            .div_ceil(ROW_MAP_CACHE_CHUNK_BLOCKS)
    }
    ...
    /// Load every chunk of this row map into the index cache concurrently. A
    /// no-op when no block cache is attached.
    pub async fn prewarm(&self) -> Result<()> {
        let Some(block_cache) = &self.block_cache else {
            return Ok(());
        };
        futures::stream::iter(0..self.num_chunks())
            .map(Ok)
            .try_for_each_concurrent(ROW_MAP_PREWARM_CONCURRENCY, |chunk_idx| async move {
                self.chunk_labels(block_cache, chunk_idx).await.map(|_| ())
            })
            .await
    }
...
mod tests {
    ...
    async fn test_block_cache_matches_uncached() {
        ...
        // Prewarm loads every chunk; subsequent reads still translate correctly.
        reader.prewarm().await.unwrap();
        assert_eq!(
            reader.translate_many(&rows).await.unwrap(),
            map.expected,
            "translation after prewarm"
        );
