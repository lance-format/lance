...
//!
//! The lifecycle feature protects deleted namespace manifests from clients that
//! would otherwise ignore the deleted marker and publish new objects.
...
pub(crate) const NAMESPACE_LIFECYCLE: u64 = 1;

const READER_KNOWN_FLAGS: u64 = NAMESPACE_LIFECYCLE;
const WRITER_KNOWN_FLAGS: u64 = NAMESPACE_LIFECYCLE;
...
mod tests {
    ...
    fn any_unknown_flag_is_refused() {
        // Unassigned feature bits must be refused.
        assert!(!can_read_manifest(2));
        assert!(!can_write_manifest(2));
