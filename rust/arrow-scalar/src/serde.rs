        ...
        other => {
            ...
            match parts.as_slice() {
                ["w", num_bytes] => {
                    ...
                    // A negative width reaches arrow's array builders as an
                    // unchecked `i32` and panics there (e.g. when a null array
                    // allocates `len * width` bytes), so reject it at the parse
                    // boundary instead of letting a corrupt type string abort.
                    if n < 0 {
                        return Err(ArrowError::InvalidArgumentError(format!(
                            "FixedSizeBinary byte count must be non-negative, got {n}"
                        )));
                    }
...
mod tests {
    ...
    #[rstest]
    #[case::negative("w:-1")]
    #[case::int_min("w:-2147483648")]
    fn test_fixed_size_binary_rejects_negative_width(#[case] fmt: &str) {
        // A negative width would reach arrow's builders as an unchecked i32 and
        // panic there; parsing must return an error instead.
        let err = format_string_to_data_type(fmt).unwrap_err();
        assert!(
    ...
    #[test]
    fn test_fixed_size_binary_accepts_zero_width() {
        assert_eq!(
            format_string_to_data_type("w:0").unwrap(),
