...
async fn rechunk_row_lineage(
        ...
        .zip(new_last_updated_at)
    {
        ...
        let placed = place_row_lineage(dataset, &lineage).await?;
        fragment.row_id_meta = Some(placed.row_ids);
        fragment.created_at_version_meta = Some(placed.created_at);
        fragment.last_updated_at_version_meta = Some(placed.last_updated_at);
    }
