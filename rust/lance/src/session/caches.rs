...
impl CacheKey for RowIdSequenceKey<'_> {
    ...
    fn write_key(&self, builder: &mut KeyBuilder) {
        ...
        match self.row_id_meta {
            ...
            // The path is a fresh random name per spilled fragment, so it
            ...
            RowIdMeta::Column(data_file) => {
                ...
                builder.write_u64(data_file.base_id.map_or(u64::MAX, u64::from));
            }
        }
    }
}
...
pub struct RowVersionSequenceKey<'a> {
    ...
    pub field_id: i32,
    /// The file is named freshly per spilled fragment, so its path identifies
    /// the contents the way an inline sequence's digest does.
