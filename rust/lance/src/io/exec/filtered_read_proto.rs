...
async fn fr_options_from_proto(
    ...
    // refine filter alone would build options no local caller can construct.
...
mod tests {
    ...
    /// A wire message carrying a refine filter alone is rejected. `with_filter`
    /// refuses that pairing locally, but the two proto fields are read
    /// independently, so nothing else stops it arriving over the wire -- and a
    /// refine filter drops rows that the read would otherwise promise to return.
    #[tokio::test]
