...
//! format's 200 KiB inline budget. Both arms measure the case for the design
//! and the costs it adds:
//!
...
async fn run_arm(dir: &str, config: Config, scenario: Scenario, spill: bool) -> ArmResult {
    ...
    let spilled_fragments = dataset
        ...
        .filter(|fragment| {
            matches!(
                fragment.metadata().row_id_meta,
                Some(lance_table::format::RowIdMeta::Column)
            )
        })
        .count();
...
fn bench_rowid_spill(_c: &mut Criterion) {
    ...
    for scenario in scenarios_from_env() {
        ...
        // a compacted fragment's sequence always stays inline however large it
        // grows. The opted-in arm takes the format's documented 200 KiB inline
        // budget.
