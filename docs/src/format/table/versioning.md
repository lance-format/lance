...
| 4096     | `FLAG_CLUSTERING_METADATA`     | No              | Yes             | The manifest carries [clustering configuration or fragment layout markers](clustering.md). One shared capability, independent of provider and layout version. Reserved, not supported until writer preservation and invalidation are implemented. Readers may ignore the metadata. |
