...
- If the fragment no longer exists, the declared cells are changed. A concurrent rewrite that
  ...
  required.
- If any data file carrying a declared field differs from the read version — replaced, removed, or
  with the declared field added to or dropped from it — the declared cells are changed. Files that
  carry no declared field may be added or removed (for example by a concurrent column addition)
  without affecting the declared cells.
