...
and row version sequences. This allows small sequences to stay in the manifest (fewer
...
form as the inline arms. No Lance writer emits them. Lance readers can load
`external_row_ids`, but no reader loads `external_created_at_versions` or
`external_last_updated_at_versions`: a fragment using either fails to open.
