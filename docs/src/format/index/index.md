...
- `covering_fields`: the trailing subset of `fields` whose values the index carries alongside its own
  ...
  index that carries no extra columns. Every id in `covering_fields` names a top-level field.
  Covering a struct column carries the whole struct, its children included, as one column. This
  declaration is not authoritative for what the segment can serve -- see
